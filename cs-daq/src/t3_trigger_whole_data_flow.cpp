    ...
    for (auto it = du_events.begin(); it != du_events.end(); )
    {
        ...
        while (it != du_events.end() && (it->first - windowStart) <= paras.m_timeWindow)
        {
            ...
            if (paras.m_t3TriggerType==2)
            {
                ...
                // else
                // {
                //     // if (distance_it == inner_distance_map->end())
                //     // {
                //         // cout << "No antenna " << it->second << " for antenna " << du_id << " in the antenna distances map" << endl;
                //     // }
                //     // else
                //         // cout << "Did not pass the casualty test " << du_id << " " << it->second << " " << (it->first - windowStart) << " " << distance_it->second*1.5 << endl;
                // }
            }
