...
void T3Trigger::triggerAlgorithm(char *data, int sz, uint64_t tmID) {
    ...
    for(int i=0; i<num; i++) {
        ...
        for(int j=i; j<num; j++) {
            ...
            if(t2-t1>paras.m_timeWindow) {
                ...
                // printf("%d timestamps within the timewindow.\n", diff);               
    ...
    for(size_t i=0; i<nStruct; i++) { // read every struct buf
        ...
        int k=0;
        // printf("set trigger DUs number: %d\n", paras.m_triggerThreshold);
        if(n>=paras.m_triggerThreshold) {
            ...
            if(msg.size()+sizeof(uint32_t)>2048000) {
