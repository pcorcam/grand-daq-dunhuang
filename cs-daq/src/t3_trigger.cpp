...
T3Trigger::T3Trigger( TriggerDone triggerDoneCallback, ZMQClient *client) {
    // std::cout << "T3Trigger::T3Trigger" << std::endl;
    ...
    // std::cout << "T3Trigger::T3Trigger end" << std::endl;
}
...
void T3Trigger::stop() {
    // std::cout << "T3Trigger::stop()" << std::endl;
    doLastT3Trigger();
    // std::cout << "T3Trigger::stop() end" << std::endl;
}
...
void T3Trigger::processData(std::string du, char *data, size_t sz) {
    ...
    // std::cout << "T3Trigger::processData()" << std::endl;
    m_daqMode = CSSysConfig::instance()->appConfig().daqMode;
    if(m_daqMode == 2) {
        // std::cout << "doT3Trigger" << std::endl;
        doT3Trigger(du, data, sz);
        // std::cout << "doT3Trigger end" << std::endl;
    }
    if(m_daqMode == 3) {
        doT3Trigger(du, data, sz);
    }
    // std::cout << "T3Trigger::processData() end" << std::endl;
}
...
void T3Trigger::doTrigger(TriggerDone cb) {
    // std::cout << "T3Trigger::doTrigger" << std::endl;
    if(cb) {
        cb();
    }
    // std::cout << "T3Trigger::doTrigger end" << std::endl;
}
...
void T3Trigger::doT3Trigger(std::string du, char* data, size_t sz) {
    std::cout << "do T3Trigger" << std::endl;
    int szData = sz-12;
    ...
    m_eventInfo[tmId][duId].buf[T3TIMEBUF_EACHDU_MAP_SZ] = {0};
        ...
        if(tmCpu - it_2->second > paras.m_timeOut) { // Second level.
            ...
            for(it_1 = it->second.begin(); it_1 != it->second.end(); it_1++) {
                ...
                if(tmpDUid>0 && it_2->first>0) {
                    ...
                    printf("tmId: %lld, DU: %d, trigger rate: %dHz\n", it_2->first, it_1->first, m_eventInfo[it_2->first][it_1->first].InfoSz/20);
            ...
            // m_tmJudgement.erase(it_2->first);
            delCounter++;
        }
        else {
            break;
    ...
    std::cout << "do T3Trigger end" << std::endl;
}

void T3Trigger::doLastT3Trigger() {
    // std::cout << "doLastT3Trigger" << std::endl;
    int eachSz = 0;
    int delCounter = 0;
    int erase_n1;
    ...
    memset(m_t3TimeBuf, 0, T3TIMEBUF_EACHDU_ALLSZ);
    // std::cout << "doLastT3Trigger end" << std::endl;
}
...
void T3Trigger::triggerAlgorithm(char *data, int sz, uint64_t tmID) {
    // std::cout << "T3Trigger::triggerAlgorithm" << std::endl;
    int num = sz/EACH_DATA_SZ;
    const int dataSz = sz;
    uint64_t t1=0, t2=0;
    uint64_t t_be=0, t_af=0;
    std::vector<uint64_t> m_t3Container(num);
    std::vector<char> m_t3TimeContainer(sz);
    char tmp[EACH_DATA_SZ]={0};
    ...
    for(size_t i=0; i<num; i++) {
        ...
        *(m_t3Container.data()+i) = atoll(tmp);
    }

    t_be = XXClock::nowNanoSeconds();
    std::sort(m_t3Container.begin(), m_t3Container.end());
    ...
    for(size_t i=0; i<num; i++) {
        snprintf(m_t3TimeContainer.data() +i*EACH_DATA_SZ, EACH_DATA_SZ, "%lld", m_t3Container[i]);
    }
    
    for(int i=0; i<num; i++) {
        memset(p,0,EACH_DATA_SZ);
        memcpy(p, m_t3TimeContainer.data() +i*EACH_DATA_SZ, EACH_DATA_SZ);
        t1 = atoll(p);
        ...
        if(num - i == paras.m_triggerThreshold) {
            ...
            for(int j=i; j<num; j++) {
                ...
                memcpy(p2, m_t3TimeContainer.data()+j*EACH_DATA_SZ, EACH_DATA_SZ);
                t2 = atoll(p2);
            ...
            if( N_counts==paras.m_triggerThreshold ){
                ...
                memcpy(win[nStruct].buf, m_t3TimeContainer.data() +N_first_position_in_last_part*EACH_DATA_SZ,(N_counts+1)*EACH_DATA_SZ);        
                ...
                printf("m_tag is %d\n", atol(tmpTag));
                sprintf(duTriggerNumbers, "%d", N_counts);
        ...
        for(int j=i; j<num; j++) {
            ...
            if(t2-t1>paras.m_timeWindow) {
                ...
                memcpy(win[nStruct].buf, m_t3TimeContainer.data() +i*EACH_DATA_SZ, diff*EACH_DATA_SZ);
                win[nStruct].sz = diff*EACH_DATA_SZ;

                ////////// ***** debug area **** //////////
                // if(diff>3) {
                //     std::cout << "diff: " << diff << std::endl;
                //     std::cout << "nStruct: " << nStruct << std::endl;
                //     for(int i2=0; i2<diff; i2++) {
                //         char testbuf[20];
                //         memcpy(testbuf, win[nStruct].buf+i2*EACH_DATA_SZ, EACH_DATA_SZ);
                //         printf("time %d: %lld\n", i2, atoll(testbuf));
                //     }
                // }
                ////////// ***** debug area **** //////////
                for(size_t j2=0; j2<win[nStruct].sz/EACH_DATA_SZ; j2++) { 
                    std::string str1;
                    str1.assign(win[nStruct].buf+j2*EACH_DATA_SZ, EACH_DATA_SZ);
                ...
                if(n>=paras.m_triggerThreshold) {
                    ...
                    printf("nEvent: %d, nTriggered: %d\n", atol(tmpTag), n);
                    // for(itt = m_trigerDU.begin(); itt != m_trigerDU.end(); itt++){
                    //     memset(testtmstamp, 0, 20);
                    //     memcpy(testtmstamp, win[nStruct].buf + k*EACH_DATA_SZ, EACH_DATA_SZ);
                    //     // fprintf(fp, "triggered DU id is %d, No.%d timestamp is %lld\n", itt->first, k, atoll(testtmstamp));
                    //     printf( "triggered DU id: %d, No.%d timestamp is %lld\n", itt->first, k, atoll(testtmstamp));
                    //     k++;
                    // }
                    // ****************** test area, 20250311 ************************* //
                    for(int i2=0; i2<diff; i2++) {
                        std::string str1;
                        str1.assign(win[nStruct].buf+i2*EACH_DATA_SZ, EACH_DATA_SZ);
                        memset(testtmstamp, 0, 20);
                        memcpy(testtmstamp, win[nStruct].buf + i2*EACH_DATA_SZ, EACH_DATA_SZ);
                        for(it_t3 = m_eventInfo[tmID].begin(); it_t3 != m_eventInfo[tmID].end(); it_t3++) {
                            std::string str2;
                            str2.assign(it_t3->second.buf, it_t3->second.InfoSz);
                            int ret = str2.find(str1, 0);
                            if(ret >= 0) {
                                printf( "triggered DU id: %d, No.%d timestamp is %lld\n", it_t3->first, i2, atoll(testtmstamp));
                            }
                        }  
                    }
                    // ***************************************************************** //
                    ...
                    m_client->writeAll(buf, szofMsg);
                    // std::cout << 1215 << std::endl;
                    i = j-1; 
                    // std::cout << "i = j-1;, i: "<< i << std::endl;
                }
    ...
    // duanbh modify max loop counts
    // for (int i = 0; i < nStruct; i++) {
    //     if(win[i].buf){
    //         delete win[i].buf;
    //         win[i].buf = nullptr; // no floating pointers
    //     }
    // }

    // std::cout << "T3Trigger::triggerAlgorithm end" << std::endl;
}