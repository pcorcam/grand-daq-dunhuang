...
EXTERNAL_LIB_PC="libyaml-cpp.so libstdc++.so.6 libyaml-cpp.so.0.7 libyaml-cpp.so.0.7.0 libzmq.so  libzmq.so.5.2.6 libzmq.so.5 libzmq.so.5.2.4"
EXTERNAL_LIB_ARM="libstdc++.so.6.0.24 libstdc++.so.6 libstdc++.so libgcc_s.so.1 libgcc_s.so libzmq.so.5.2.6 libzmq.so libzmq.so.5 libzmq.so.5.2.4"
